...
Владелец разрешил по умолчанию выполнить merge task-matching PR и штатный
protected production deployment без нового запроса на каждом шаге, **только**
когда одновременно выполнены все условия:

- есть отдельная согласованная задача, в явную область которой включены
...
Это разрешение не применяется к фактическому deployment, пока для конкретного
production-окружения не выполнена предусмотренная проверка, включая успешный
deployment preflight непосредственно перед изменяющим шагом. Успешный PR не
...
Постоянное разрешение **не** распространяется на неоднозначную область задачи
или проблему безопасности, destructive/recovery-действия, миграции, импорт,
изменение production-данных, классификации, прав доступа либо секретов, а также
