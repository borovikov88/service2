...
def daily_config():
    try:
        ...
        if org < 1 or (user is not None and user < 1) or not 1 <= months <= 24 or (enabled and user is None):
            raise ValueError
    except (AttributeError, TypeError, ValueError, ZoneInfoNotFoundError):
        raise DailyConfigError(
            "Проверьте организацию, пользователя, время, часовой пояс и период ONEC_FINANCE_* .".replace("* .", "*.")
        ) from None
...
def _select_run(config, now):
    with transaction.atomic():
        ...
        slot = _schedule_slot(local)
        previous_slot = OneCODataSyncRun.objects.filter(
            organization=organization,
            mode=OneCODataSyncRun.MODE_AUTO_APPLY,
            sync_scope___schedule_slot=slot,
        ).order_by("-created_at").first()
        if previous_slot:
            if (
                previous_slot.status == OneCODataSyncRun.STATUS_COMPLETED
                and (previous_slot.progress or {}).get("finance_position_state") == "retryable_error"
            ):
                return previous_slot, "finance_position_retry"
            return None, "already_attempted"

        full_previous = OneCODataSyncRun.objects.filter(
            organization=organization,
            mode=OneCODataSyncRun.MODE_AUTO_APPLY,
            sync_scope___schedule_day=day,
        ).order_by("-created_at").first()
        full_due = local.time() >= config.at and full_previous is None
        months = config.months if full_due else 1
        ...
        run, _ = start_unified_sync(
            ...
            SUPPORTED_REPORT_TYPES,
            mode=OneCODataSyncRun.MODE_AUTO_APPLY,
            period_start=_add_months(end, -(months - 1)),
            period_end=end,
            schedule_day=day if full_due else None,
        )
        scope = dict(run.sync_scope or {})
        scope["_schedule_slot"] = slot
        run.sync_scope = scope
        run.save(update_fields=["sync_scope"])
        return run, "scheduled_full" if full_due else "scheduled_hourly"
...
def finalize_finance_position_step(run, now=None):
    ...
    if run.status != OneCODataSyncRun.STATUS_COMPLETED:
        return progress.get("finance_position_state")
    if progress.get("finance_position_state") in {"completed", "failed"}:
        return progress.get("finance_position_state")
    ...
    try:
        ...
        _position_note(
            run.pk,
            state="failed",
            error="Баланс и расчёты не обновлены; предыдущий снимок сохранён.",
        )
        return "failed"
    try:
    ...
    _position_note(
        run.pk,
        state="completed",
        finance_position_snapshot_id=snapshot.pk,
        finance_position_snapshot_at=snapshot.snapshot_at.isoformat(),
        finance_position_fetched_at=snapshot.fetched_at.isoformat(),
    )
    return "completed"
...
def worker_tick(*, max_steps=4, max_seconds=240, now=None):
    ...
    visible_state = (
        position_state
        if run.status == OneCODataSyncRun.STATUS_COMPLETED
        and position_state in {"retryable_error", "failed"}
        else run.status
    )
    return {
        "state": visible_state,
