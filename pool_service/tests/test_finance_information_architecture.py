...
class FinanceInformationArchitectureTests(TestCase):
    ...
    def _data_capabilities(
        self, *, gross_profit=False, payroll=False, mapping=False,
        cashflow=False, cashflow_view=False,
    ):
        capabilities = {
            ...
            "can_view_cashflow": cashflow_view,
        }
        data_access = any((gross_profit, payroll, mapping, cashflow))
        section_access = data_access or cashflow_view
        with ExitStack() as stack:
            ...
            stack.enter_context(
                patch(
                    "pool_service.finance_views.can_access_finance_section",
                    return_value=section_access,
                )
            )
            yield
    ...
    def test_every_finance_data_capability_renders_a_usable_destination(self):
        cases = (
            ...
            ({"cashflow": True}, "finance_onec_import_list"),
    ...
    def test_cashflow_import_enables_finance_data_and_destination(self):
        self.client.force_login(self.conceptual)
        ...
        with self._data_capabilities(cashflow=True):
            ...
            self.assertRedirects(
                self.client.get(reverse("finance_dashboard")),
                reverse("finance_data"),
                fetch_redirect_response=False,
            )
            response = self.client.get(reverse("finance_data"))
            self.assertEqual(response.status_code, 200)
            self.assertContains(response, reverse("finance_onec_import_list"))

    def test_cashflow_view_enables_section_and_analytics_navigation(self):
        self.client.force_login(self.conceptual)
        with self._data_capabilities(cashflow_view=True):
            self.assertTrue(
                finance_service.can_access_finance_section(
                    self.conceptual, self.organization
                )
            )
            self.assertRedirects(
                self.client.get(reverse("finance_dashboard")),
        ...
        analytics = {
            ...
            for item in navigation[0]["items"]
        }
        self.assertEqual(analytics, {"ДДС": "finance_onec_cashflow_dashboard"})

    def test_cashflow_import_routes_activate_data_navigation(self):
        route_names = (
        ...
        for route_name in route_names:
            with self.subTest(route_name=route_name), self._data_capabilities(
                cashflow=True
            ):
                navigation = finance_navigation(
                    self.conceptual, self.organization, current_route=route_name
                )
                data_items = navigation[2]["items"]
                self.assertEqual(len(data_items), 1)
                self.assertTrue(data_items[0]["active"])
